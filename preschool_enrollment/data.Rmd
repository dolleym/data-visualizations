...
<h2>Assign variables human readable names</h2>
```{r message=FALSE}
variables <- c(
...
assemble <- lapply(geos, function(geo){
  if(geo == 'county') {  
    data <- tidycensus::get_acs(
      ...
      survey = "acs5",
      cache_table = TRUE
    ) %>% filter(GEOID == county_of_interest) } else if (geo == 'state') {
      data <- tidycensus::get_acs(
        geography = geo,
        variables = variables,
        state = state_of_interest,
...
<h2>Preview and Save Chart Data</h2>
```{r echo=FALSE}
...
# Save chart
ggsave("output/bar_chart_enrollment2.png", height = 2.75)
```
```{r echo=FALSE}
